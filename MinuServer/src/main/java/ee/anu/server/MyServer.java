...
public class MyServer {
    ...
    public static void serve(RequestHandler requestHandler, int port) throws IOException {
    ...
    public static void processRequestAndRespond(Socket clientSocket, RequestHandler requestHandler) throws IOException {
        ...
        RawRequest bytes = inputStreamToByteArray(clientSocket.getInputStream());
        ...
        Request request = new Request(requestURI, parseParameters(req), bytes.getRequestBody(), method);

        Response responseData;
        try {
            responseData = requestHandler.handleRequest(request);
        } catch (Exception e){
            responseData = getInternalErrorResponse(e);
        }
        processRequest(responseData, clientSocket);
    }

    private static Response getInternalErrorResponse(Exception e) throws UnsupportedEncodingException {
        e.printStackTrace();
        Map<String, String> headers = new HashMap<>();
        byte[] data = "Internal server error".getBytes("UTF-8");
        headers.put("Content-Type", "text/html; charset=UTF-8");
        headers.put("Content-Length", String.valueOf(data.length));
        return new Response(headers, data, Status.InternalError);
    }

    private static RawRequest inputStreamToByteArray(InputStream stream) throws IOException {
        ...
        return new RawRequest(headeriteVoolikuOts.toByteArray(), bodyVoolikuOts.toByteArray());
    }
